...
## 2026-08-05 — v5.20 文章质量优化（内行手册+导语强化+示例标注）

- 用户问文章质量 75 分如何优化；瓶颈：材料有限、偶发硬编、专业感不足
- 修复（prompt 层，零风险）：
  ① 煤矿安全内行手册注入（一通三防/顶板/瓦斯/机电运输/水害/班组/应急 7 大环节标准做法，材料不足时按内行框架展开）
  ② 导语强化（第一句点出与读者关系+严重性）
  ③ 假设场景必须标注"示例"（防硬编）
- 端到端实测（汛期水害主题）：专业术语命中 5/7（排水/边坡/避灾等），"地表水倒灌井下""矸石山坍塌"等内行表述，质量提升明显

## 2026-08-05 — v5.19.1 新人引导提示（模拟验收发现）

- 模拟验收（新人视角）：功能全通，但首次打开无引导、素材区无可点选提示
- 修复：素材区顶部加步骤式引导（①获取素材 → ②点选素材 → ③自动生成），手机友好（小字换行）

## 2026-08-05 — v5.19 文章结构结尾多样化（用户要求不千篇一律）

- 用户反馈：文章结构/结尾固定套路（如每篇结尾都是「现在就去检查XX」）
- 修复：prompt 第 5 条改为——小节 3-6 个数量排布每次变化；结尾 5 种方式（行动号召/警示总结/温情叮嘱/数据收尾/提问思考）每次变换
- 端到端实测 2 篇对比：标题不同 ✅ 结尾不同 ✅（承诺式 vs 对照细则式）

## 2026-08-05 — v5.18.1 补充素材严格过滤（用户批评：不相关就不该带）

- 用户批评：六必讲只是举例，代码里不该处处带它；生成文章不相关的素材不该混入
- 修复：① 补充素材过滤升级（同分类且 ≥2 词命中 或 ≥3 词跨分类）② 主题词拆双字词匹配 + 词上限 8→14（碎词占名额问题）③ 清理注释残留
- 验证（真实素材库）：选「非煤矿山监管培训班」素材 → 补充 3 条全是隐患排查主题（实施细则/行动指南/排查治理年），六必讲已排除 ✅

## 2026-08-05 — v5.18 选中素材定主题（修复文章被补充素材带偏）

- 用户反馈：选「隐患排查治理实施细则」素材生成，文章却以「六必讲」为主题（AI 被摘要更长的补充素材带偏）
- 根因：素材库补充逻辑混入同主题素材，AI 自动选更有料的当主线，用户选中的素材成配角
- 修复：选中素材标记 isPrimary，prompt 标注【★主题素材】+ 指令「文章必须围绕它展开，其余仅背景参考」
- 验证：模拟用户场景端到端实测——标题「隐患不销号等于白查」，主线为隐患排查，六必讲降为背景小节 ✅

## 2026-08-05 — v5.17.1 DDG 素材来源标记

- 素材库验证（8/5 03:01 轮）：来源明显多样化（30+ 来源），DDG 新货已进来，但无来源标记（显示"?"）
- 修复：searchDuckDuckGo 从解码后的真实链接提取域名做 source（失败标 DuckDuckGo），测试 2/2 全过
- 下一轮抓取后素材来源可清晰区分

## 2026-08-04 — v5.17 管道新增 DuckDuckGo 免费搜索层

- 用户要求：稳定长期的免费搜索方案（查证 2026 开源社区主流：SearXNG 自建 + DuckDuckGo 免 Key）
- 实现（scrape-safety-news.js）：新增 searchDuckDuckGo（html.duckduckgo.com/html/ 解析 + uddg 链接解码 + 失败容错），管道第三层 5 个通用安全主题词（每词 2s 间隔）
- 解析器单测 5/5 全过（标题/摘要/链接解码/500 不崩）；管道语法 ✅
- 效果：素材库新增免费搜索来源（无 Key 无额度，长期稳定）；本环境网络无法直测 DDG，真实效果待下一轮 GitHub Actions 抓取验证

## 2026-08-04 — v5.16 免费搜索通道（完全免费替代博查）

- 用户要求：完全免费（博查 1000 次/3 个月有额度限制）；模型不能自己上网，需程序内置联网搜索工具
- 实测：rss2json 中转 Google News RSS 关键词搜索可用（5-10 条/次，内容对口，完全免费）
- 实现：新增 searchFreeWeb（Google News 搜索 + 失败重试一次）
  - 生成时搜索：无博查 Key → 自动走免费通道（静默，不提示）
  - 实时素材：无博查 Key → 主题词走 Google News 免费搜索（比通用 RSS 对口），失败回退通用 RSS
- 局限：摘要为标题+来源（几十字），无长摘要；链接为 Google News 编码（点击可跳转原文，程序无法解码抓取——实测 protobuf 无 URL）
- 效果：不填博查 Key = 完全免费 + 联网搜索参考（标题级）；填 Key = 增强（长摘要）

## 2026-08-04 — v5.15.4 额度周期包核算修正（用户发现）

- 用户发现：博查 1000 次为周期包（7/20-10/21 共 1000 次），非每月；已用 85 次剩 915
- 重新核算：915 ÷ 78 天 ≈ 11 次/天；实时素材改 72 小时窗口 + 每词 count 2（16 次/3 天 ≈ 5.3 次/天）
- 合计约 10 次/天 × 78 天 ≈ 780 次，留 135 余量 ✅

## 2026-08-04 — v5.15.3 博查额度核算修正

- 用户质疑额度：6 小时窗口仍不够（48次×每天4次=192次/天，5天用完1000次）
- 澄清：管道每天30条抓取不用博查（Google News/Bing），不消耗额度；消耗在实时素材（48次/抓）和生成搜索
- 修正：24 小时窗口（每天最多1次抓取）+ 每词 count 6→3（24次/天）→ 月消耗约870次，1000 限额留 130 余量 ✅

## 2026-08-04 — v5.15.2 博查限流防护 + 额度控制

- 实测发现：8 词全并发触发博查 429 限流（整批失败）；每次点击消耗 48 次请求，月免费额度 1000 次会被快速耗光
- 修复：① 分批并发（每批 2 词）② 单词独立容错（429/失败只跳过该词）③ 429 等 2 秒重试一次 ④ 实时抓取 6 小时窗口（频繁点按钮不重复消耗额度，缓存复用）
- 实测：限流窗口下重试救回限流词（36 条→过滤 31 条）；额度由每次点击 48 次降至最多每 6 小时 48 次

## 2026-08-04 — v5.15.1 开滦官网内容接入实时素材

- 用户提议：把开滦集团官网的核心会议精神、重点文章加进抓取内容（面向开滦工会文职）
- 实测：博查 site:kailuan.com.cn 搜索可用，返回开滦要闻（道德讲堂/温情安全网/反三违等，带栏目和日期）
- 实现：实时素材主题词新增 3 个开滦词（site:kailuan.com.cn 安全、site:kailuan.com.cn 会议 精神、开滦集团 安全生产 部署）；开滦官网内容（sourceUrl 含 kailuan.com.cn）直接保留不过滤

## 2026-08-04 — v5.15 实时素材升级为博查搜索 + 语气防雷同（执行方：Claude Code）

### 背景（用户反馈）
- 实时素材（RSS 通用频道过滤）数量少（2-3 条）、与管道素材无区分、鸡肋
- 用户确认：博查按主题搜索可行（"拿着标题去搜"的理解正确）；担心同一语气文章雷同

### 实现
- **实时素材主通道 = 博查按 5 个安全主题词搜索**（煤矿安全班组、安全生产隐患排查应急、防汛高温职工健康、矿山事故警示演练、职业病尘肺体检），每词 6 条 → 去重 + 安全过滤 → 25 条左右，全部安全主题、来源多样（政府/企业/媒体）、摘要 100-200 字/条
- RSS 通用频道降级为回退（无博查 Key 或博查失败时）
- **语气防雷同**：四档语气指令统一附加"开头切入方式要有变化（场景/数据/提问/新闻事件式）+ 结构不重复"，避免同语气文章雷同

### 验证
- 真实博查 Key 实测：30 条 → 去重 25 条，全安全主题 ✅
- 语气变化性要求注入四档指令 ✅
- 影响范围：强安兴企安全园地生文助手.html（fetchLiveMaterials / getToneRule）

---

## 2026-08-03 — v5.14.5 清旧实时素材缓存

- 用户反馈：刷新后仍看到跑题的实时素材（缓存了过滤前的旧素材，30 天不清）
- 修复：缓存版本 gist-v7→v8 强制清缓存；新抓实时素材均为过滤后（实测 20 条留 3 条安全相关）

## 2026-08-03 — v5.14.4 语气档位拉开差异

- 用户要求：正式宣传要像新闻报道通稿风格，四档语气差别明确
- 正式宣传 = 新闻通稿风（导语式开头/第三人称/官方书面语）；接地气 = 场景故事聊天风；温馨提醒 = 谈心关怀风；班前会口吻 = 短句祈使叮嘱风
- 验证：四档文本互不相同，差异检查 4/4 全过

## 2026-08-03 — v5.14.3 语气选项真实生效

- 用户反馈：选「正式宣传」生成出来仍是接地气风（半样子货）
- 根因：系统指令写死"语言要接地气"，优先级压过用户选择的语气
- 修复：getToneRule 按所选语气生成对应指令（正式/温馨/班前会/接地气，匹配 5/5 全过），插入系统提示词第 13 条

## 2026-08-03 — v5.14.2 实时素材安全主题过滤

- 用户反馈：实时素材（中新网社会/人民网要闻为通用频道）跑题严重（旅游/鸟类/政治等杂闻）
- 新增 isSafetyRelevant 主题过滤：40 个安全关键词（煤矿/应急/消防/防汛/隐患/高温等），不命中即过滤
- 实测：20 条真实标题只保留 3 条安全相关（防台风/防汛减灾/高温度夏），14/14 测试全过

## 2026-08-03 — v5.14.1 静默降级 + 实时素材置顶

- 去掉「原文抓取受限」「搜索失败」等 toast（静默降级，后台日志保留，观感统一）
- 实时素材评分 60→90 置顶，列表加蓝色「实时」badge，一眼可见

## 2026-08-03 — v5.14 前端实时抓取通道（用户提议：复用强安视界思路，执行方：Claude Code）

### 背景
- 素材库管道在美国服务器（GitHub Actions）抓中国源站受限，正文抓取失败率高
- 用户观察：强安视界前端抓取（rss2json 通道）及时且质量高 → 提议生文助手复用该思路

### 实现（前端）
- 新增 `fetchLiveMaterials`：rss2json 实时抓取 2 个中国安全类 RSS 源（中新网-社会、人民网-要闻），实测各 10 条、摘要 119-150 字、1 秒返回
- 「获取安全素材」时并行执行：Gist 素材库 + 实时抓取，合并进素材列表（标题去重，live 素材标记 isLive，不参与"新增"徽章）
- 生成链路：live 素材直接用其长摘要生成（不再联网抓原文）
- 实测：20 条 / 1s / 字段完整 ✅

### 意义
- 素材供给双通道：Gist 管道（服务器）+ 实时抓取（用户网络），互相弥补；用户网络环境抓中国源无限制
- 不依赖任何代理/外部抓取服务（rss2json 为公共服务）
...
### 背景
- 现有 CORS 代理全部失效（SCF 502 / 部署损坏 / 公共代理不可用），前端抓原文几乎必失败 → 频繁降级提示
- 但**抓取管道（GitHub Actions 服务器端）本来就把正文存进了素材库**（enrichItem 截断 500 字，合并写 B 时 `{...item}` 保留）——只是前端拉素材时把 content 字段丢了，白存

### 修复（前端）
- 素材映射新增 `content` 字段（正文直接进素材对象）
- 生成时素材自带正文（≥200 字）→ **直接使用，不再联网抓原文**（也就不会有"抓取受限"提示）
- 缓存版本 gist-v6 → gist-v7，强制刷新拿到新数据结构
- 无正文的历史素材仍走原抓取/补充链路

### 生效节奏
- 素材库现有 91 条无正文（历史数据）；下一轮定时抓取（每日 07/13/19/01 点）后新素材带正文，逐轮累积

### 验证
- 语法 ✅；三处前端改动静态验证 ✅；管道 enrichItem/合并逻辑确认保留 content ✅
- 影响范围：强安兴企安全园地生文助手.html（fetchTodayMaterials / generateDraft）
