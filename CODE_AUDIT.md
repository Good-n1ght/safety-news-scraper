...
> **⚠️ 更正说明（2026-08-02，v5.11）**：本文档部分结论已过时或与代码不符，独立审查已逐条核实（详见 `复核裁定_2026-08-02.md` 的 D1-D9 清单）。要点：①「生文助手.html 已删除」错误——该文件一直存在，v5.11 才真正删除；②「proxy/app.js 已删除」错误——仍在仓库且是 proxy/package.json 的 main；③「B 按 URL 去重」错误——实际按标题前 20 字符去重；④ 衰减公式描述错误——实际为 `1/(1+0.03×days)` 无下限；⑤ 手机演示包"旧版/略有差异"描述已过时（7/30 后已字节级一致）。阅读时以 CHANGELOG.md（v5.11 条目）与当前代码为准。
